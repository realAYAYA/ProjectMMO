...
public:
    ...
    bool Init();
    void Foreach(const TFunction<bool(const {{class_name}}*)>& Func);
...
private:
    ...
    UPROPERTY()
    TMap<{{key_field_type}}, {{class_name}}*> Data; 
...
void {{manager_name}}::Foreach(const TFunction<bool(const {{class_name}}*)>& Func)
{
...
FString {{manager_name}}::GetConfigFileName() const
{
...
def generate_code(args, defines: list):
    ...
    manager_name = f'U{base_name}ConfigManager'
