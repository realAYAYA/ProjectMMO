...
#define M_GAME_MESSAGE_HANDLE_REGISTRANT_TYPENAME PREPROCESSOR_JOIN(_NET_MESSAGE_,PREPROCESSOR_JOIN(MMODULENAME,PREPROCESSOR_JOIN(_,__LINE__)))
...
{ \
	...
	void Handle(UMGameSession*, const PREPROCESSOR_JOIN(F,PREPROCESSOR_JOIN(RpcMethodName,Req))&, PREPROCESSOR_JOIN(F,PREPROCESSOR_JOIN(RpcMethodName,Ack))&); \
};	\
static M_GAME_MESSAGE_HANDLE_REGISTRANT_TYPENAME M_GAME_MESSAGE_HANDLE_REGISTRANT_VARNAME; \
void M_GAME_MESSAGE_HANDLE_REGISTRANT_TYPENAME::Handle(UMGameSession* GameSessionVar, const PREPROCESSOR_JOIN(F,PREPROCESSOR_JOIN(RpcMethodName,Req))& ReqVar, PREPROCESSOR_JOIN(F,PREPROCESSOR_JOIN(RpcMethodName,Ack)& AckVar))
